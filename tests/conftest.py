...
def bias_vectors(rng: np.random.Generator) -> List[np.ndarray]:
    ...
    return [rng.standard_normal(64), rng.standard_normal(32), rng.standard_normal(1)]
...
def assert_loss_properties(loss_value: float) -> None:
    ...
    assert isinstance(
        loss_value, (int, float, np.number)
    ), f"Loss must be numeric, got {type(loss_value)}"
