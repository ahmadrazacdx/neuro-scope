...
from .core import _ForwardPass, _BackwardPass
...
class MLP:
    ...
    def __init__(self, layer_dims, hidden_activation='leaky_relu', out_activation=None, init_method='smart', 
                 init_seed=42, dropout_rate=0.0, dropout_type='normal'):
    ...
    def _initialize_weights(self):
        if self.init_method == 'he':
            self.weights, self.biases = WeightInits.he_init(self.layer_dims, self.init_seed)
        elif self.init_method == 'xavier':
            self.weights, self.biases = WeightInits.xavier_init(self.layer_dims, self.init_seed)
        elif self.init_method == 'random':
            self.weights, self.biases = WeightInits.random_init(self.layer_dims, seed=self.init_seed)
        elif self.init_method == 'selu_init':
            self.weights, self.biases = WeightInits.selu_init(self.layer_dims, self.init_seed)
        elif self.init_method == 'smart':
            self.weights, self.biases = WeightInits.smart_init(self.layer_dims, self.hidden_activation, self.init_seed)
        else:
    ...
    def compile(self, optimizer='adam', lr=0.001, reg=None, lamda=0.01, gradient_clip=None):
        """
    ...
    def _print_summary(self):
        ...
        for i, (W, b) in enumerate(zip(self.weights, self.biases)):
            ...
            print(f"{f'Layer {i+1}':<12} {layer_type:<18} {output_shape:<15} {layer_params:<10}")
    ...
    def predict(self, X):
        ...
        """
        activations, _ = _ForwardPass.forward_mlp(X, self.weights, self.biases, 
    ...
    def evaluate(self, X, y, metric='smart', binary_thresh=0.5):
        ...
        """
        return self._evaluate_mlp(X, y, self.weights, self.biases, self.lamda, 
                          self.reg, self.hidden_activation, self.out_activation, 
                          binary_thresh, metric)
    
    def _evaluate_mlp(self, X, y, weights, biases, lamda, reg=None, 
                hidden_activation=None, out_activation=None, binary_thresh=0.5, metric='smart'):
        activations, z_values = _ForwardPass.forward_mlp(X, weights, biases, hidden_activation, out_activation,
                                           dropout_rate=0.0, training=False) 
    ...
    def _update_parameters_adam(self, dW, db, lr):
        ...
        for i in range(len(self.weights)):
            # Weight updates
            state['m_weights'][i] = state['beta1'] * state['m_weights'][i] + (1 - state['beta1']) * dW[i]
            state['v_weights'][i] = state['beta2'] * state['v_weights'][i] + (1 - state['beta2']) * (dW[i]**2)
            ...
            # Bias updates
            state['m_biases'][i] = state['beta1'] * state['m_biases'][i] + (1 - state['beta1']) * db[i]
            state['v_biases'][i] = state['beta2'] * state['v_biases'][i] + (1 - state['beta2']) * (db[i]**2)
    ...
    def fit(self, X_train, y_train, X_test=None, y_test=None, epochs=10, batch_size=None, 
            verbose=True, log_every=1, early_stopping_patience=50, lr_decay=None, 
            numerical_check_freq=100, metric='smart', reset_before_training=True,
            monitor=None, monitor_freq=1):
        """
        ...
        if X_test is not None:
            X_test = Utils.validate_array_input(X_test, "X_test", min_dims=2, max_dims=2)
            y_test = Utils.validate_array_input(y_test, "y_test", min_dims=1, max_dims=2)
        ...
        for epoch in range(1, epochs + 1):
            ...
            # Training loop
            for batch_idx, (Xb, yb) in enumerate(Utils.get_batches(X_train, y_train, batch_size, shuffle=True)):
                try:
                    yb = yb.reshape(-1, 1) if yb.ndim == 1 else yb
                    capture_monitor = bool(monitor and (batch_idx % monitor_stride == 0))
                    
                    # Forward pass
                    activations, z_values = _ForwardPass.forward_mlp(Xb, self.weights, self.biases, 
                                                      self.hidden_activation, self.out_activation,
                                                      dropout_rate=self.dropout_rate, 
                                                      dropout_type=self.dropout_type, training=True)
                    ...
                    if batch_idx % numerical_check_freq == 0:
                        issues = Utils.check_numerical_stability(activations, f"epoch_{epoch}_batch_{batch_idx}")
                        if issues:
                    ...
                    # Backward pass
                    dW, db = _BackwardPass.backward_mlp(yb, activations, z_values, self.weights, self.biases,
                                        Xb, self.hidden_activation, self.out_activation)
                    
                    
                    # Store gradients for monitoring
                    if capture_monitor and monitor_activations is not None:
                        monitor_gradients = dW.copy() if isinstance(dW, list) else [dW.copy()]
                    
                    # Gradient clipping
                    if self.gradient_clip is not None:
                        all_grads = dW + db
                        clipped_grads = Utils.gradient_clipping(all_grads, self.gradient_clip)
                    ...
                    for layer_idx in range(num_layers):
                        ...
                        epoch_weights_batch_data[f'layer_{layer_idx}'].extend(layer_weights)
                        # Activation statistics
                        layer_activations = activations[layer_idx].flatten()
                        epoch_activations_batch_data[f'layer_{layer_idx}'].extend(layer_activations)
                        # Gradient statistics
                        layer_gradients = dW[layer_idx].flatten()
                        epoch_gradients_batch_data[f'layer_{layer_idx}'].extend(layer_gradients)
                        # Gradient norm
                        layer_gradient_norm = np.linalg.norm(layer_gradients)
                        epoch_gradient_norms_batch_data[f'layer_{layer_idx}'].append(layer_gradient_norm)
                    ...
                    for layer_idx in range(num_layers):
                        ...
                        update_norm = np.linalg.norm(prev_weights_[layer_idx] - self.weights[layer_idx])
                        if weight_norm > 1e-12:  
                            update_ratio = update_norm / weight_norm
                        else:
                            update_ratio = 0.0
                        epoch_weight_update_ratios_batch_data[f'layer_{layer_idx}'].append(update_ratio)
                    ...
                    if capture_monitor and monitor_activations is not None and prev_weights is not None:
                        monitor_weight_updates = [prev - curr for prev, curr in zip(prev_weights, self.weights)]
                except Exception as batch_error:
                    epoch_errors += 1
                    if epoch_errors <= 3:
                        warnings.warn(f"Batch {batch_idx} error: {str(batch_error)[:100]}")
                    continue
            ...
            for layer_idx in range(num_layers):
                ...
                if epoch_weights_batch_data[layer_key]:
                    ...
                    weight_stats_over_epochs[layer_key]['mean'].append(np.mean(weight_data))
                    weight_stats_over_epochs[layer_key]['std'].append(np.std(weight_data))
                else:
                ...
                if epoch_activations_batch_data[layer_key]:
                    activation_data = np.abs(np.array(epoch_activations_batch_data[layer_key]))
                    activation_stats_over_epochs[layer_key]['mean'].append(np.mean(activation_data))
                    activation_stats_over_epochs[layer_key]['std'].append(np.std(activation_data))
                else:
                ...
                if epoch_gradients_batch_data[layer_key]:
                    gradient_data = np.abs(np.array(epoch_gradients_batch_data[layer_key]))
                    gradient_stats_over_epochs[layer_key]['mean'].append(np.mean(gradient_data))
                    gradient_stats_over_epochs[layer_key]['std'].append(np.std(gradient_data))
                else:
                ...
                if epoch_weight_update_ratios_batch_data[layer_key]:
                    batch_ratios = np.array(epoch_weight_update_ratios_batch_data[layer_key])
                    # Store mean of batch ratios as the representative ratio for this epoch
                    weight_update_ratios_over_epochs[layer_key].append(np.mean(batch_ratios))
                else:
                    weight_update_ratios_over_epochs[layer_key].append(0.0)
                
                # Collect representative samples for distribution plots
                if epoch_activations_batch_data[layer_key]:
                    activation_samples = np.array(epoch_activations_batch_data[layer_key])
                    if len(activation_samples) > max_samples_per_epoch:
                        indices = np.random.choice(len(activation_samples), max_samples_per_epoch, replace=False)
                        activation_samples = activation_samples[indices]
                    epoch_distribution_data['activations'][layer_key].append(activation_samples)
                
                if epoch_gradients_batch_data[layer_key]:
                    gradient_samples = np.array(epoch_gradients_batch_data[layer_key])
                    if len(gradient_samples) > max_samples_per_epoch:
                        indices = np.random.choice(len(gradient_samples), max_samples_per_epoch, replace=False)
                        gradient_samples = gradient_samples[indices]
                    epoch_distribution_data['gradients'][layer_key].append(gradient_samples)
                
                if epoch_weights_batch_data[layer_key]:
                    weight_samples = np.array(epoch_weights_batch_data[layer_key])
                    if len(weight_samples) > max_samples_per_epoch:
                        indices = np.random.choice(len(weight_samples), max_samples_per_epoch, replace=False)
            ...
            if monitor and epoch % monitor_freq == 0:
                try:
                    ...
                    if monitor_activations is not None:
                        L = len(self.weights)
                        last_act = self.out_activation if self.out_activation is not None else 'linear'
            ...
            if verbose and (epoch % log_every == 0 or epoch == 1 or epoch == epochs):
                ...
                if X_test is not None:
                    print(f"Epoch {epoch:3d}  Train loss: {train_loss:.6f}, Train {metric_name}: {train_acc:.4f} "
                          f"Val loss: {test_loss:.7f}, Val {metric_name}: {test_acc:.5f}{lr_info}")
                else:
                    print(f"Epoch {epoch:3d}  Train loss: {train_loss:.6f}, Train {metric_name}: {train_acc:.4f}{lr_info}")
    ...
    def fit_batch(self, X_batch, y_batch, epochs=10, verbose=True, metric='smart'):
        ...
        n = min(8, max(2, X_batch.shape[0] // 10)) if X_batch.shape[0] >= 10 else X_batch.shape[0]
        X_batch = X_batch[:n]
        ...
        for epoch in range(epochs):
            ...
            activations, z_values = _ForwardPass.forward_mlp(
                ...
                self.hidden_activation, self.out_activation, training=True
            )
            
            # Backward pass
            dW, db = _BackwardPass.backward_mlp(
                y_batch, activations, z_values, self.weights, self.biases,
                X_batch, self.hidden_activation, self.out_activation
            )
