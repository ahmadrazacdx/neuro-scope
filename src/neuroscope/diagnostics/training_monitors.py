...
class TrainingMonitor:
    ...
    def monitor_relu_dead_neurons(self, activations: List[np.ndarray], activation_functions: Optional[List[str]] = None) -> Tuple[float, str]:
        """
        ...
        activation_thresholds = {
            ...
            'relu': {'dead_threshold': 0.90, 'warning_threshold': 0.75, 'zero_tolerance': 1e-8},
            # Leaky ReLU: Allows small negative values, lower natural sparsity
            # Dead threshold: >85% (Maas et al. 2013 recommendations)
            'leaky_relu': {'dead_threshold': 0.85, 'warning_threshold': 0.70, 'zero_tolerance': 1e-6},
            'default': {'dead_threshold': 0.85, 'warning_threshold': 0.70, 'zero_tolerance': 1e-7}
        }
        ...
        for i, activation in enumerate(activations[:-1]):
            ...
            if activation.ndim > 1:
                ...
                zero_ratios = np.mean(np.abs(activation) <= thresholds['zero_tolerance'], axis=0)
            else:
                layer_neurons = 1
                zero_ratios = np.array([np.mean(np.abs(activation) <= thresholds['zero_tolerance'])])
            layer_dead = np.sum(zero_ratios > thresholds['dead_threshold'])
            total_neurons += layer_neurons
            dead_neurons += layer_dead
        dead_percentage = (dead_neurons / total_neurons * 100) if total_neurons > 0 else 0.0
    ...
    def monitor_vanishing_gradients(self, gradients: List[np.ndarray]) -> Tuple[float, str]:
        """
    ...
    def monitor_exploding_gradients(self, gradients: List[np.ndarray]) -> Tuple[float, str]:
        """
    ...
    def monitor_learning_progress(self, current_loss: float, val_loss: Optional[float] = None) -> Tuple[float, str]:
        """
    ...
    def monitor_overfitting(self, train_loss: float, val_loss: Optional[float] = None) -> Tuple[float, str]:
        """
        ...
        if len(train_losses) >= 5 and len(val_losses) >= 5:
            ...
            # Calculate trends
            train_trend = (recent_train[-1] - recent_train[0]) / (recent_train[0] + 1e-8)
    ...
    def monitor_activation_saturation(self, activations: List[np.ndarray], 
                                    activation_functions: List[str] = None) -> Tuple[float, str]:
        """
        ...
        for i, activation in enumerate(activations[:-1]):
            ...
            activation_func = activation_functions[i] if activation_functions and i < len(activation_functions) else "unknown"
            ...
            if activation_func.lower() in ['tanh', 'sigmoid']:
                ...
                if activation_func.lower() == 'tanh':
                    ...
                    gradient_dead = np.mean((activation_flat < 0.05) | (activation_flat > 0.95))
                    
                gradient_score = gradient_dead
            else:
                gradient_score = 0.0
            layer_saturation = (extreme_saturation * 0.5 + 
                              distribution_score * 0.3 + 
                              gradient_score * 0.2)
    ...
    def monitor_plateau(self, current_loss: float, val_loss: Optional[float] = None, 
                       current_gradients: Optional[List[np.ndarray]] = None) -> Tuple[float, str]:
        """
        ...
        stagnation_scores = []
        for window, name in [(short_window, "short"), (medium_window, "medium"), (long_window, "long")]:
            if len(window) < 3:
                continue
    ...
    def monitor_weight_update_ratio(self, weights: List[np.ndarray], 
                                    weight_updates: List[np.ndarray]) -> Tuple[float, str]:
        """
    ...
    def monitor_step(self, epoch: int, train_loss: float, val_loss: Optional[float] = None,
                    ...
                    activation_functions: Optional[List[str]] = None) -> Dict[str, Any]:
        """
        ...
        results = {}
        if activations:
            dead_pct, dead_status = self.monitor_relu_dead_neurons(activations, activation_functions)
            saturation_score, saturation_status = self.monitor_activation_saturation(activations, activation_functions)
            results['dead_neurons'] = {'value': dead_pct, 'status': dead_status}
            results['saturation'] = {'value': saturation_score, 'status': saturation_status}
        else:
