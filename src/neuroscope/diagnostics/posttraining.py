...
class PostTrainingEvaluator:
    ...
    def evaluate_robustness(self, X: np.ndarray, y: np.ndarray, 
        ...
        try:
            ...
            return {'status': 'ERROR', 'error': str(e), 'note': "Failed to compute baseline performance"}
        robustness_scores = []
        for noise_level in noise_levels:
            try:
                ...
                if baseline_predictions.shape[1] > 1:
                    consistency = np.mean(np.argmax(baseline_predictions, axis=1) == np.argmax(noisy_predictions, axis=1))
                else:
                    consistency = max(0, np.corrcoef(baseline_predictions.flatten(), noisy_predictions.flatten())[0, 1])
                
                accuracy_robustness = max(0, 1 - (accuracy_drop / (baseline_accuracy + 1e-8)))
    ...
    def evaluate_performance(self, X: np.ndarray, y: np.ndarray) -> Dict[str, Any]:
        ...
        try:
            ...
            samples_per_second = X.shape[0] / prediction_time if prediction_time > 0 else float('inf')
            total_params = sum(w.size + b.size for w, b in zip(self.model.weights, self.model.biases))
            ...
            return {'status': 'ERROR', 'error': str(e), 'note': "Failed to evaluate performance"}
    
    def _evaluate_all_metrics(self, X: np.ndarray, y: np.ndarray, predictions: np.ndarray) -> Dict[str, float]:
        ...
        try:
            ...
            if is_classification:
                if is_binary:
                    ...
                    metrics_results.update({
                        'accuracy_multiclass': Metrics.accuracy_multiclass(y, predictions),
    ...
    def evaluate_stability(self, X: np.ndarray, y: np.ndarray, n_samples: int = 100) -> Dict[str, Any]:
        """Evaluate prediction stability across similar inputs. (K Neighbor Approach)"""
        try:
            if X.shape[0] < 2:
                return {'status': 'ERROR', 'error': 'Insufficient data', 'note': 'Need at least 2 samples'}
            ...
            for idx in test_indices:
                ...
                if pred_ref.shape[1] > 1:
                    stability = 1.0 if np.argmax(pred_ref) == np.argmax(pred_neighbor) else 0.0
                else:
                    pred_distance = np.abs(pred_ref.flatten()[0] - pred_neighbor.flatten()[0])
            ...
            return {'status': 'ERROR', 'error': str(e), 'note': "Failed to evaluate stability"}
    ...
    def evaluate(self, X_test: np.ndarray, y_test: np.ndarray,
        ...
        if X_train is not None and y_train is not None:
            evaluations.append(("Generalization", lambda: self.evaluate_generalization(X_train, y_train, X_test, y_test)))
        ...
        for eval_name, eval_func in evaluations:
            try:
                ...
                score_keys = ['overall_robustness', 'accuracy', 'generalization_score', 'overall_stability']
                score = next((f"{result[key]:.3f}" for key in score_keys if key in result), "N/A")
                
                print(f"{eval_name:<15} {status:<12} {score:<12} {note:<45}")
                
            except Exception as e:
                all_results[eval_name] = {'status': 'ERROR', 'error': str(e), 'note': 'Evaluation failed'}
                print(f"{eval_name:<15} {'ERROR':<12} {'N/A':<12} {'Evaluation failed':<45}")
        ...
        if 'Performance' in all_results and all_results['Performance'].get('status') != 'ERROR':
    ...
    def _display_metrics_evaluation(self, performance_result: Dict[str, Any]):
        ...
        is_classification = any(metric in all_metrics for metric in 
                              ['accuracy_binary', 'accuracy_multiclass', 'precision', 'recall', 'f1_score'])
        ...
        if is_classification:
            metric_order = [('accuracy_binary', 'Accuracy'), ('accuracy_multiclass', 'Accuracy'),
                          ('precision', 'Precision'), ('recall', 'Recall'), ('f1_score', 'F1-Score')]
            def get_status(key, value):
                return ("EXCELLENT", "Outstanding performance") if value >= 0.95 else \
                       ("PASS", "Good performance") if value >= 0.85 else \
                       ("WARN", "Moderate performance") if value >= 0.70 else \
                       ("FAIL", "Poor performance")
        else:
            metric_order = [('r2_score', 'R² Score'), ('mae', 'Mean Absolute Error'),
                          ('mse', 'Mean Squared Error'), ('rmse', 'Root Mean Squared Error')]
            def get_status(key, value):
                if key == 'r2_score':
                    return ("EXCELLENT", "Outstanding fit") if value >= 0.9 else \
                           ("PASS", "Good fit") if value >= 0.7 else \
                           ("WARN", "Moderate fit") if value >= 0.5 else \
                           ("FAIL", "Poor fit")
                else:
                    return ("EXCELLENT", "Very low error") if value <= 0.1 else \
                           ("PASS", "Low error") if value <= 0.3 else \
                           ("WARN", "Moderate error") if value <= 0.5 else \
                           ("FAIL", "High error")
        
        metrics_to_display = []
        for metric_key, display_name in metric_order:
            if metric_key in all_metrics and isinstance(all_metrics[metric_key], (int, float)):
        ...
        metrics_pass_count = status_counts.get('PASS', 0) + status_counts.get('EXCELLENT', 0)
        total_metrics = len(metrics_to_display)
        
        metrics_overall = "METRICS ISSUES DETECTED" if status_counts.get('FAIL', 0) > 0 else \
                         "SOME METRICS WARNINGS" if status_counts.get('WARN', 0) > 0 else \
