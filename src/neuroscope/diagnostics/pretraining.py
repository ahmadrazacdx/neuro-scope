...
class PreTrainingAnalyzer:
    ...
    def analyze_initial_loss(self, X: np.ndarray, y: np.ndarray) -> Dict[str, Any]:
        ...
        try:
            ...
            is_binary = len(unique_targets) == 2 and set(unique_targets).issubset({0, 1})
            is_multiclass = len(unique_targets) > 2 and all(isinstance(x, (int, np.integer)) for x in unique_targets)
            ...
            if is_binary or is_multiclass:
                status = "PASS" if 0.8 <= ratio <= 1.5 else "WARN" if 0.5 <= ratio <= 2.0 else "FAIL"
            else:
        ...
        for i, (W, b) in enumerate(zip(self.model.weights, self.model.biases)):
            ...
            if actual_init_method == 'random':
                ...
                if activation in ['relu', 'leaky_relu']:
                    expected_std = np.sqrt(2.0 / fan_in)  # He initialization (He et al. 2015)
                    init_type = "He"
                elif activation in ['tanh', 'sigmoid']:
                    expected_std = np.sqrt(2.0 / (fan_in + fan_out))  # Xavier initialization (Glorot & Bengio 2010)
                    init_type = "Xavier" 
                elif activation == 'selu':
                    expected_std = np.sqrt(1.0 / fan_in)  # Lecun normal (Lecun et al. 1998)
                    init_type = "Lecun"
                else:
                    expected_std = np.sqrt(2.0 / (fan_in + fan_out))  # Xavier uniform (Glorot & Bengio 2010)
    ...
    def analyze_layer_capacity(self) -> Dict[str, Any]:
        ...
        for i in range(len(layer_dims) - 2):  
            ...
            elif next_dim < current_dim * 0.2:  # Significant bottleneck (80%+ reduction)
                bottlenecks.append({
                    'layer': i + 1,
                    'from_dim': current_dim,
                    'to_dim': next_dim,
    ...
    def analyze_architecture_sanity(self) -> Dict[str, Any]:
        ...
        if input_dim > 5000:  
            warnings.append("Very high input dimensionality - consider dimensionality reduction")
        elif input_dim < 2:
        ...
        if hasattr(self.model, 'hidden_activation'):
            if self.model.hidden_activation == 'sigmoid' and depth > 3:
                issues.append("Sigmoid in deep networks causes vanishing gradients (Bengio et al. 2009)")
    ...
    def analyze_capacity_data_ratio(self, X: np.ndarray, y: np.ndarray) -> Dict[str, Any]:
        ...
        return {
            ...
            'status': status,
            'note': self._get_data_ratio_note(params_per_sample, n_samples, total_params)
        }
    
    def analyze_convergence_feasibility(self, X: np.ndarray, y: np.ndarray) -> Dict[str, Any]:
        """Assess whether the current setup can theoretically converge."""
        feasibility_score = 0
        max_score = 6
        issues = []
        try:
            from mlp.core import _ForwardPass, _BackwardPass
            activations, z_values = _ForwardPass.forward_mlp(
                X[:min(32, len(X))], self.model.weights, self.model.biases,
                self.model.hidden_activation, self.model.out_activation, training=True
            )
            
            dW, _ = _BackwardPass.backward_mlp(
                y[:min(32, len(y))].reshape(-1, 1) if y.ndim == 1 else y[:min(32, len(y))],
                activations, z_values, self.model.weights, self.model.biases,
                X[:min(32, len(X))], self.model.hidden_activation, self.model.out_activation
            )
        ...
        try:
            initial_loss, _ = self.model.evaluate(X[:min(100, len(X))], y[:min(100, len(y))])
    ...
    def _get_data_ratio_note(self, params_per_sample: float, n_samples: int, total_params: int) -> str:
