...
class Visualizer:
    ...
    def plot_learning_curves(self, figsize=(9, 4), ci=False, markers=True, save_path=None, metric='accuracy'):
        """
        ...
        marker_settings = {
            'train': {'marker': 'o', 'markersize': 3, 'markevery': 2} if markers else {'marker': None},
            'val': {'marker': 's', 'markersize': 3, 'markevery': 2} if markers else {'marker': None}
        }
        ax1.plot(epochs, self.history["train_loss"], label='Training', 
                 linewidth=1.2, color='#1f77b4', **marker_settings['train'])
        ax1.plot(epochs, self.history["test_loss"], label='Validation', 
                 linewidth=1.2, color='#ff7f0e', **marker_settings['val'])
        if ci and len(epochs) > 3:
            self._add_confidence_intervals(ax1, epochs, self.history["train_loss"], '#1f77b4')
            self._add_confidence_intervals(ax1, epochs, self.history["test_loss"], '#ff7f0e')
        ...
        ax2.plot(epochs, self.history["train_acc"], label='Training', 
                 linewidth=1.2, color='#1f77b4', **marker_settings['train'])
        ax2.plot(epochs, self.history["test_acc"], label='Validation', 
                 linewidth=1.2, color='#ff7f0e', **marker_settings['val'])
        if ci and len(epochs) > 3:
            self._add_confidence_intervals(ax2, epochs, self.history["train_acc"], '#1f77b4')
            self._add_confidence_intervals(ax2, epochs, self.history["test_acc"], '#ff7f0e')
    ...
    def plot_activation_hist(self, epoch=None, figsize=(9, 4), kde=False, last_layer=False, save_path=None):
        """
        ...
        """
        return self._plot_epoch_distribution('activations', 'Activation Distributions (Epoch-Agg)', 
                                           'Activation Value', epoch, figsize, kde, last_layer, save_path)
    
    def plot_gradient_hist(self, epoch=None, figsize=(9, 4), kde=False, last_layer=False, save_path=None):
        """
        ...
        """
        return self._plot_epoch_distribution('gradients', 'Gradient Distributions (Epoch-Agg)', 
                                           'Gradient Value', epoch, figsize, kde, last_layer, save_path)
    
    def plot_weight_hist(self, epoch=None, figsize=(9, 4), kde=False, last_layer=False, save_path=None):
        ...
        """
        return self._plot_epoch_distribution('weights', 'Weight Distributions (Epoch-Agg)', 
                                           'Weight Value', epoch, figsize, kde, last_layer, save_path)

    def _plot_epoch_distribution(self, data_type, title, xlabel, epoch, figsize, kde, last_layer, save_path):
        """
        ...
        if num_epochs == 0:
            ax.text(0.5, 0.5, f'No {data_type} data collected during training', 
                   transform=ax.transAxes, ha='center', va='center', fontsize=12)
            plt.show()
            return
        ...
        for i, (layer_key, color) in enumerate(zip(layers_to_plot, layer_colors)):
            if target_epoch < len(data_dict[layer_key]):
                ...
                if len(epoch_samples) > 0:
                    ...
                    if kde:
                        counts, bin_edges = np.histogram(epoch_samples, bins=bins, density=True)
                        bin_centers = (bin_edges[:-1] + bin_edges[1:]) / 2
                        x_smooth = np.linspace(bin_centers.min(), bin_centers.max(), len(bin_centers) * 3)
                        y_smooth = np.interp(x_smooth, bin_centers, counts)
                        ax.plot(x_smooth, y_smooth, color=color, 
                               linewidth=2.0, alpha=0.9, label=f'Layer {i+1} (n={len(epoch_samples)})')
                        ax.fill_between(x_smooth, y_smooth, alpha=0.3, color=color)
                    else:
                        # Standard histogram
                        ax.hist(epoch_samples, bins=bins, density=True, alpha=0.7, 
                               color=color, label=f'Layer {i+1} (n={len(epoch_samples)})',
                               edgecolor='black', linewidth=0.5)
        if data_type in ['gradients', 'weights']:
            ax.axvline(0, color='red', linestyle='--', alpha=0.8, linewidth=1.5, label='Zero line')
    ...
    def plot_activation_stats(self, activation_stats=None, figsize=(12, 4), save_path=None, reference_lines=False):
        """
        ...
        Args:
            ...
            save_path: Path to save figure
        """
        fig, ax = self._configure_plot('Activation Statistics Over Epochs', 'Epoch', 'Mean Activation', figsize)
        data_source = activation_stats if activation_stats is not None else self.activation_stats_over_epochs
        if not data_source:
            ax.text(0.5, 0.5, 'No activation stats data available\nPlease run model.fit() to generate statistics', 
                   transform=ax.transAxes, ha='center', va='center', fontsize=12)
            plt.show()
            return
        ...
        for i, (layer_name, color) in enumerate(zip(data_source.keys(), layer_colors)):
            if has_mean_std:
                ...
                ax.plot(epochs, mean_values, 
                       label=f'Layer {i+1} Mean', color=color, 
                       linewidth=self.line_style['width'], alpha=self.line_style['alpha'],
                       marker='o', markersize=self.line_style['markersize'], 
                       markevery=self.line_style['markevery'], linestyle='-')
                std_values = data_source[layer_name]['std']
                ax.plot(epochs, std_values, 
                       label=f'Layer {i+1} Std', color=color, 
                       linewidth=self.line_style['width'], alpha=self.line_style['alpha']*0.7,
                       marker='s', markersize=self.line_style['markersize'], 
                       markevery=self.line_style['markevery'], linestyle='--')
            else:
                y_values = data_source[layer_name]
                ax.plot(epochs, y_values, 
                       label=f'Layer {i+1}', color=color, 
                       linewidth=self.line_style['width'], alpha=self.line_style['alpha'],
                       marker='o', markersize=self.line_style['markersize'], 
        ...
        if reference_lines:
            ax.axhline(y=0.01, color=self.colors['reference']['good'], linestyle='--', alpha=0.7, label='Healthy Min (0.01)')
            ax.axhline(y=5.0, color=self.colors['reference']['good'], linestyle='--', alpha=0.7, label='Healthy Max (5.0)')
        ax.legend(fontsize=9, bbox_to_anchor=(1.02, 1), loc='upper left')
        plt.tight_layout()
        if save_path:
            plt.savefig(save_path, dpi=300, bbox_inches='tight', facecolor='white')
    ...
    def plot_gradient_stats(self, figsize=(12, 4), save_path=None, reference_lines=False):
        """Plot gradient statistics over time with both mean and std."""
        fig, ax = self._configure_plot('Gradient Statistics Over Epochs', 'Epoch', 'Mean |Gradient|', figsize)
        if not self.gradient_stats_over_epochs:
            ax.text(0.5, 0.5, 'No gradient stats data available\nPlease run model.fit() to generate statistics', 
                   transform=ax.transAxes, ha='center', va='center', fontsize=12)
            plt.show()
            return
        ...
        for i, (layer_name, color) in enumerate(zip(self.gradient_stats_over_epochs.keys(), layer_colors)):
            if has_mean_std:
                ...
                ax.plot(epochs, std_values, 
                       label=f'Layer {i+1} Std', color=color, 
                       linewidth=self.line_style['width'], alpha=self.line_style['alpha']*0.7,
                       marker='D', markersize=self.line_style['markersize'], 
                       markevery=self.line_style['markevery'], linestyle='--')
            else:
                y_values = self.gradient_stats_over_epochs[layer_name]
                ax.plot(epochs, y_values, label=f'Layer {i+1}', color=color, 
                       linewidth=self.line_style['width'], alpha=self.line_style['alpha'],
                       marker='s', markersize=self.line_style['markersize'], 
                       markevery=self.line_style['markevery'])
        ...
        if reference_lines:
            ax.axhline(y=1e-4, color=self.colors['reference']['good'], linestyle='--', alpha=0.7, label='Healthy Min (1e-4)')
            ax.axhline(y=1e-1, color=self.colors['reference']['good'], linestyle='--', alpha=0.7, label='Healthy Max (1e-1)')
        ax.set_yscale('log')
        ax.legend(fontsize=9, bbox_to_anchor=(1.02, 1), loc='upper left')
        plt.tight_layout()
        if save_path:
    ...
    def plot_weight_stats(self, figsize=(12, 4), save_path=None, reference_lines=False):
        ...
        fig, ax = self._configure_plot('Weight Statistics Over Epochs', 'Epoch', 'Mean |Weight|', figsize)
        ...
        for i, (layer_name, color) in enumerate(zip(self.weight_stats_over_epochs.keys(), layer_colors)):
            if has_mean_std:
                mean_values = self.weight_stats_over_epochs[layer_name]['mean']
                ax.plot(epochs, mean_values, 
                       label=f'Layer {i+1} Mean', color=color, 
                       linewidth=self.line_style['width'], alpha=self.line_style['alpha'],
                       marker='^', markersize=self.line_style['markersize'], 
                       markevery=self.line_style['markevery'], linestyle='-')
                std_values = self.weight_stats_over_epochs[layer_name]['std']
                ax.plot(epochs, std_values, 
                       label=f'Layer {i+1} Std', color=color, 
                       linewidth=self.line_style['width'], alpha=self.line_style['alpha']*0.7,
                       marker='v', markersize=self.line_style['markersize'], 
                       markevery=self.line_style['markevery'], linestyle='--')
            else:
        ...
        if reference_lines:
            ax.axhline(y=0.05, color=self.colors['reference']['good'], linestyle='--', alpha=0.7, label='Healthy Min (0.05)')
            ax.axhline(y=0.8, color=self.colors['reference']['good'], linestyle='--', alpha=0.7, label='Healthy Max (0.8)')
        ax.legend(fontsize=9, bbox_to_anchor=(1.02, 1), loc='upper left')
        plt.tight_layout()
        if save_path:
            plt.savefig(save_path, dpi=300, bbox_inches='tight', facecolor='white')
    ...
    def plot_update_ratios(self, update_ratios=None, figsize=(12, 4), save_path=None, reference_lines=False):
        """
        ...
        Args:
            ...
            save_path: Path to save figure
        """
        fig, ax = self._configure_plot('Weight Update Ratios Over Epochs', 'Epoch', 'Update Ratio', figsize)
        data_source = update_ratios if update_ratios is not None else self.weight_update_ratios
        if not data_source:
            ax.text(0.5, 0.5, 'No weight update ratio data available\nPlease run model.fit() to generate statistics', 
                   transform=ax.transAxes, ha='center', va='center', fontsize=12)
            plt.show()
            return
        ...
        if reference_lines:
            ax.axhline(y=1e-4, color=self.colors['reference']['good'], linestyle='--', alpha=0.7, label='Healthy Min (1e-4)')
            ax.axhline(y=1e-2, color=self.colors['reference']['good'], linestyle='--', alpha=0.7, label='Healthy Max (1e-2)')
        ax.set_yscale('log')
        ax.legend(fontsize=9, bbox_to_anchor=(1.02, 1), loc='upper left')
        if save_path:
            plt.savefig(save_path, dpi=300, bbox_inches='tight', facecolor='white')
    ...
    def plot_gradient_norms(self, gradient_norms=None, figsize=(12, 4), save_path=None, reference_lines=False):
        """Plot gradient norms per layer over epochs."""
        fig, ax = self._configure_plot('Gradient Norms Over Epochs', 'Epoch', 'Gradient Norm', figsize)
        data_source = gradient_norms if gradient_norms is not None else self.gradient_norms
        if not data_source:
        ...
        if reference_lines:
            ax.axhline(y=1.0, color=self.colors['reference']['good'], linestyle='--', alpha=0.7, label='Clipping Threshold (1.0)')
            ax.axhline(y=0.01, color=self.colors['reference']['warning'], linestyle='--', alpha=0.7, label='Vanishing Gradient Warning (0.01)')
        ax.set_yscale('log')
        ax.legend(fontsize=9, bbox_to_anchor=(1.02, 1), loc='upper left')
        if save_path:
            plt.savefig(save_path, dpi=300, bbox_inches='tight', facecolor='white')
    ...
    def plot_training_animation(self, bg='dark', save_path=None):
        ...
        colors = {
            'primary': '#2E86AB', 'secondary': "#F18F01", 'accent': "#28D631", 
            'success': "#FF2727", 'secondary_accent': "#FFC062", 'primary_accent': "#63CBF8",
        ...
        for ax in axes:
            style_axis(ax)
        train_loss, test_loss = self.history.get('train_loss', []), self.history.get('test_loss', [])
        train_acc, test_acc = self.history.get('train_acc', []), self.history.get('test_acc', [])
        ...
        def animate(frame):
            ...
            # 1. Loss Evolution
            axes[0].set_title('Loss Evolution', color=colors['text'], fontweight='bold', fontsize=15)
            if len(train_loss) >= current_epoch:
                train_losses_current = train_loss[:current_epoch]
                axes[0].plot(epochs_range, train_losses_current, 
                           color=colors['primary'], linewidth=2.5, label='Train Loss', alpha=0.9)
            else:
                train_losses_current = []
            if len(test_loss) >= current_epoch:
                test_losses_current = test_loss[:current_epoch]
                axes[0].plot(epochs_range, test_losses_current, 
                           color=colors['secondary'], linewidth=2.5, label='Val Loss', alpha=0.9)
            else:
                test_losses_current = []
            ...
            if train_losses_current and test_losses_current:
                ...
                axes[0].fill_between(epochs_common, train_common, test_common, 
                                   where=np.array(test_common) > np.array(train_common),
                                   color=colors['secondary_accent'], alpha=0.4, interpolate=True)
                
                axes[0].fill_between(epochs_common, train_common, test_common,
                                   where=np.array(test_common) <= np.array(train_common), 
                                   color=colors['primary_accent'], alpha=0.3, interpolate=True)
            ...
            axes[1].set_title('Accuracy Evolution', color=colors['text'], fontweight='bold', fontsize=16)
            if len(train_acc) >= current_epoch:
                axes[1].plot(epochs_range, train_acc[:current_epoch], 
                           color=colors['primary'], linewidth=2, label='Train Acc')
            if len(test_acc) >= current_epoch:
                axes[1].plot(epochs_range, test_acc[:current_epoch], 
            ...
            # 3. Current Metrics Bar Chart
            axes[2].set_title('Current Metrics', color=colors['text'], fontweight='bold', fontsize=16)
            metrics = ['Train Loss', 'Val Loss', 'Train Acc', 'Val Acc']
            data_sources = [train_loss, test_loss, train_acc, test_acc]
            values = [data[current_epoch - 1] if len(data) >= current_epoch else 0 
            ...
            for i, (value, color) in enumerate(zip(values, bar_colors)):
                if value > 0:
                    axes[2].bar(i, value, width=0.6, color=color, alpha=0.7, edgecolor='none')
                    axes[2].text(i, value + max_val * 0.02, f'{value:.3f}', 
            ...
            # 4. Gradient Flow Analysis
            axes[3].set_title('Gradient Flow Across Layers', color=colors['text'], fontweight='bold', fontsize=16)
            if (hasattr(self, 'gradient_stats_over_epochs') and self.gradient_stats_over_epochs and 
                current_epoch > 0):
                layer_colors = [colors['primary'], colors['secondary'], colors['accent']]
                
                for i, (layer_key, layer_stats) in enumerate(self.gradient_stats_over_epochs.items()):
                    if isinstance(layer_stats, dict) and 'mean' in layer_stats:
                        if current_epoch <= len(layer_stats['mean']):
                            ...
                            axes[3].plot(epochs_range, gradient_means, 
                                       color=color, linewidth=2, alpha=0.8, 
                                       label=f'Layer {i}', marker='o', markersize=3)
                ...
                axes[3].axhline(y=1e-7, color=colors['success'], linestyle=':', alpha=0.5, linewidth=1)
                axes[3].axhline(y=1e-2, color=colors['success'], linestyle=':', alpha=0.5, linewidth=1)
            else:
                axes[3].text(0.5, 0.5, 'Gradient Flow Analysis\n(Gradient stats not available)', 
                           ha='center', va='center', transform=axes[3].transAxes, 
                           color=colors['text'], fontsize=12, alpha=0.7)
            fig.suptitle(f'MLP Training Animation - Epoch {current_epoch}/{epochs}', 
                        fontsize=22, fontweight='bold', color=colors['text'])
        
        # Create and save animation
        anim = animation.FuncAnimation(fig, animate, frames=epochs, interval=1000//fps, repeat=True)
